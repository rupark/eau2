...
public:
    ...
    virtual void _guessFieldType(StrSlice slice, size_t field_num) {
        ...
        if (_typeGuesses[field_num] != Provider::ColumnType::STRING) {
            if (is_numeric && !has_dot) {
                // If it's an integer (not float), check if it's 0 or 1, which would indicate a
                // bool column
                int val = slice.toInt();
                if ((val == 0 || val == 1) && _typeGuesses[field_num] != Provider::ColumnType::INTEGER &&
                    _typeGuesses[field_num] != Provider::ColumnType::FLOAT) {
                    ...
                    _typeGuesses[field_num] = Provider::ColumnType::BOOL;
                } else if (_typeGuesses[field_num] != Provider::ColumnType::FLOAT) {
                    // Use integer guess only if we didn't already guess float (which could not be
                    // parsed as integer)
                    _typeGuesses[field_num] = Provider::ColumnType::INTEGER;
                }
                ...
                _typeGuesses[field_num] = Provider::ColumnType::STRING;
            }
        }
    }
    ...
    */
    ...
    virtual void guessSchema() {
        ...
        for (size_t i = 0; i < _num_columns; i++) {
            _typeGuesses[i] = Provider::ColumnType::UNKNOWN;
        }
    ...
    char getCharFromProvColType(Provider::ColumnType prov_typ) {
        switch (prov_typ) {
